...
def phase_space_distance(h1, h2, hist1, hist2, pairs_1, pairs_2,
                         min_snaps=4):
    n_snap = h1.shape[1]
    snap = np.arange(n_snap)
    n_pair = len(pairs_1)

    dist = np.zeros(n_pair)
    x_norm = np.mean([h1["rvir"][0,-1], h2["rvir"][0,-1]])
    v_norm = np.mean([h1["vmax"][0,-1], h2["vmax"][0,-1]])

    pre_mpeak_1 = pre_infall_mpeak(h1, hist1)
    pre_mpeak_2 = pre_infall_mpeak(h2, hist2)

    for i in range(n_pair):
        i1, i2 = pairs_1[i], pairs_2[i]

        if not (0.5 < pre_mpeak_1[i1] / pre_mpeak_2[i2] < 2):
            dist[i] = np.inf
            continue
        ...
        infall_snap_1 = hist1["first_infall_snap"][i1] if i1 > 0 else n_snap
        infall_snap_2 = hist2["first_infall_snap"][i2] if i2 > 0 else n_snap
        infall_snap = max(infall_snap_1, infall_snap_2)

        merger_snap_1 = hist1["merger_snap"][i1] / h1.shape[1]
        merger_snap_2 = hist2["merger_snap"][i2] / h2.shape[1]

        ok = (h1["ok"][i1] & h2["ok"][i2] &
              (snap < infall_snap_1) & (snap < infall_snap_2))
        
        n_ok = np.sum(ok)
        if n_ok < min_snaps:
            dist[i] = np.inf
            continue
        dx = h1[i1,ok]["x"] - h2[i2,ok]["x"]
        dv = h1[i1,ok]["v"] - h2[i2,ok]["v"]
        dm = np.log10(h1[i1,ok]["mvir"]) - np.log10(h2[i2,ok]["mvir"])
        dist[i] = np.sqrt(np.sum(dm**2)/n_ok + np.sum(dx**2)/(3*n_ok) + 
                          (merger_snap_1-merger_snap_2)**2)

    return dist

def match_subhaloes_old(h1, h2, hist1, hist2, target_idx_1, target_idx_2):
    matched_1, matched_2 = {}, {}

    pairs_1_1, pairs_1_2 = np.meshgrid(target_idx_1, np.arange(len(h2),dtype=int))
    pairs_2_1, pairs_2_2 = np.meshgrid(np.arange(len(h1), dtype=int),target_idx_2)
    pairs_1 = np.hstack([pairs_1_1.flatten(), pairs_2_1.flatten()])
    pairs_2 = np.hstack([pairs_1_2.flatten(), pairs_2_2.flatten()])
    
    dist = phase_space_distance(h1, h2, hist1, hist2, pairs_1, pairs_2)
    ok = dist < np.inf
    pairs_1, pairs_2, dist = pairs_1[ok], pairs_2[ok], dist[ok]

    order = np.argsort(dist)
    dist = dist[order]

    match_1_to_2 = np.ones(len(h1), dtype=int)*-1
    match_2_to_1 = np.ones(len(h1), dtype=int)*-1

    pairs_1, pairs_2 = pairs_1[order], pairs_2[order]

    n_pairs = len(pairs_1)
    for i in range(n_pairs):
        if dist[i] == np.inf: break
        i1, i2 = pairs_1[i], pairs_2[i]
        if i1 not in matched_1 and i2 not in matched_2:
            matched_1[i1], matched_2[i2] = None, None
            match_1_to_2[i1] = i2
            match_2_to_1[i2] = i1
