    ...
    for snap in range(np.min(starting_snap[targets]), max_snap + 1):
        ...
        for j in range(len(targets)):
            ...
            for i in range(2):
                ...
                # Plot host
                symlib.plot_circle(ax[i], 0, 0, host_rvir,
                                 c="k", lw=3, ls="--")

                # Plot the rockstar location
                symlib.plot_circle(ax[i], h["x"][i_sub,snap,dim_x],
                                 h["x"][i_sub,snap,dim_y], r_rs,
                                 c=pc("b"), lw=2)
            
                # Plot infall tracking
                symlib.plot_circle(ax[i], x_t1[dim_x], x_t1[dim_y], r_t1,
                                 c="w", lw=2)
                ...
                symlib.plot_circle(ax[i], x_t2[dim_x], x_t2[dim_y], r_t2,
                                 c="w", lw=2, ls="--")
